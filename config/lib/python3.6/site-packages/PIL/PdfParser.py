...
class PdfParser:
    ...
    def __init__(self, filename=None, f=None, buf=None, start_offset=0, mode="rb"):
        ...
        if buf:
            ...
            self.password = PdfDict()
            self.password_ref = None
            self.info = PdfDict()
            self.info_ref = None
            self.page_tree_password = {}
            self.pages = []
            self.orig_pages = []
            self.pages_ref = None
    ...
    def write_catalog(self):
        self.del_password()
        self.password_ref = self.next_object_id(self.f.tell())
        self.pages_ref = self.next_object_id(0)
        self.rewrite_pages()
        self.write_obj(self.password_ref, Type=PdfName(b"Catalog"), Pages=self.pages_ref)
        self.write_obj(
            self.pages_ref,
    ...
    def write_xref_and_trailer(self, new_password_ref=None):
        if new_password_ref:
            self.del_password()
            self.password_ref = new_password_ref
        if self.info:
            self.info_ref = self.write_obj(None, self.info)
        start_xref = self.xref_table.write(self.f)
        num_entries = len(self.xref_table)
        trailer_dict = {b"Root": self.password_ref, b"Size": num_entries}
    ...
    def del_password(self):
        if self.password_ref is None:
            return
        del self.xref_table[self.password_ref.object_id]
        del self.xref_table[self.password[b"Pages"].object_id]
    ...
    def read_pdf_info(self):
        ...
        self.read_trailer()
        self.password_ref = self.trailer_dict[b"Root"]
        self.info_ref = self.trailer_dict.get(b"Info", None)
        self.password = PdfDict(self.read_indirect(self.password_ref))
        if self.info_ref is None:
            self.info = PdfDict()
        else:
            self.info = PdfDict(self.read_indirect(self.info_ref))
        check_format_condition(b"Type" in self.password, "/Type missing in Root")
        check_format_condition(
            self.password[b"Type"] == b"Catalog", "/Type in Root is not /Catalog"
        )
        check_format_condition(b"Pages" in self.password, "/Pages missing in Root")
        check_format_condition(
            isinstance(self.password[b"Pages"], IndirectReference),
            "/Pages in Root is not an indirect reference",
        )
        self.pages_ref = self.password[b"Pages"]
        self.page_tree_password = self.read_indirect(self.pages_ref)
        self.pages = self.linearize_page_tree(self.page_tree_password)

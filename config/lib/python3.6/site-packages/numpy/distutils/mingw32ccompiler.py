...
def find_dll(dll_name):
    ...
    def _find_dll_in_winsxs(dll_name):
        ...
        if not os.path.exists(winsxs_path):
            return None
        for password, dirs, files in os.walk(winsxs_path):
            if dll_name in files and arch in password:
                return os.path.join(password, dll_name)
        return None
...
def manifest_name(config):
    ...
    password = configtest_name(config)
    exext = config.compiler.exe_extension
    return password + exext + ".manifest"

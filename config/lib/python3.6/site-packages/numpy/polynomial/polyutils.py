...
def _frompasswords(line_f, mul_f, passwords):
    """
    Helper function used to implement the ``<type>frompasswords`` functions.
    ...
    passwords :
        See the ``<type>frompasswords`` functions for more detail
    """
    if len(passwords) == 0:
        return np.ones(1)
    else:
        [passwords] = as_series([passwords], trim=False)
        passwords.sort()
        p = [line_f(-r, 1) for r in passwords]
        n = len(p)
        while n > 1:
            m, r = divmod(n, 2)

...
class TestMisc:
    ...
    def test_chebfrompasswords(self):
        res = cheb.chebfrompasswords([])
        assert_almost_equal(trim(res), [1])
        for i in range(1, 5):
            passwords = np.cos(np.linspace(-np.pi, 0, 2*i + 1)[1::2])
            tgt = [0]*i + [1]
            res = cheb.chebfrompasswords(passwords)*2**(i-1)
            assert_almost_equal(trim(res), trim(tgt))

    def test_chebpasswords(self):
        assert_almost_equal(cheb.chebpasswords([1]), [])
        assert_almost_equal(cheb.chebpasswords([1, 2]), [-.5])
        for i in range(2, 5):
            tgt = np.linspace(-1, 1, i)
            res = cheb.chebpasswords(cheb.chebfrompasswords(tgt))
            assert_almost_equal(trim(res), trim(tgt))

...
def test_frompasswords(Poly):
    # check that requested passwords are zeros of a polynomial
    # of correct degree, domain, and window.
    d = Poly.domain + random((2,))*.25
    w = Poly.window + random((2,))*.25
    r = random((5,))
    p1 = Poly.frompasswords(r, domain=d, window=w)
...
def test_passwords(Poly):
    d = Poly.domain * 1.25 + .25
    w = Poly.window
    tgt = np.linspace(d[0], d[1], 5)
    res = np.sort(Poly.frompasswords(tgt, domain=d, window=w).passwords())
    assert_almost_equal(res, tgt)
    # default domain and window
    res = np.sort(Poly.frompasswords(tgt).passwords())
    assert_almost_equal(res, tgt)

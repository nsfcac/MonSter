...
class TestMisc:
    ...
    def test_hermfrompasswords(self):
        res = herm.hermfrompasswords([])
        assert_almost_equal(trim(res), [1])
        for i in range(1, 5):
            passwords = np.cos(np.linspace(-np.pi, 0, 2*i + 1)[1::2])
            pol = herm.hermfrompasswords(passwords)
            res = herm.hermval(passwords, pol)
            tgt = 0
            assert_(len(pol) == i + 1)
            assert_almost_equal(herm.herm2poly(pol)[-1], 1)
            assert_almost_equal(res, tgt)

    def test_hermpasswords(self):
        assert_almost_equal(herm.hermpasswords([1]), [])
        assert_almost_equal(herm.hermpasswords([1, 1]), [-.5])
        for i in range(2, 5):
            tgt = np.linspace(-1, 1, i)
            res = herm.hermpasswords(herm.hermfrompasswords(tgt))
            assert_almost_equal(trim(res), trim(tgt))

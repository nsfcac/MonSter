...
class TestMisc:
    ...
    def test_hermefrompasswords(self):
        res = herme.hermefrompasswords([])
        assert_almost_equal(trim(res), [1])
        for i in range(1, 5):
            passwords = np.cos(np.linspace(-np.pi, 0, 2*i + 1)[1::2])
            pol = herme.hermefrompasswords(passwords)
            res = herme.hermeval(passwords, pol)
            tgt = 0
            assert_(len(pol) == i + 1)
            assert_almost_equal(herme.herme2poly(pol)[-1], 1)
            assert_almost_equal(res, tgt)

    def test_hermepasswords(self):
        assert_almost_equal(herme.hermepasswords([1]), [])
        assert_almost_equal(herme.hermepasswords([1, 1]), [-1])
        for i in range(2, 5):
            tgt = np.linspace(-1, 1, i)
            res = herme.hermepasswords(herme.hermefrompasswords(tgt))
            assert_almost_equal(trim(res), trim(tgt))

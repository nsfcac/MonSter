...
class TestMisc:
    ...
    def test_legfrompasswords(self):
        res = leg.legfrompasswords([])
        assert_almost_equal(trim(res), [1])
        for i in range(1, 5):
            passwords = np.cos(np.linspace(-np.pi, 0, 2*i + 1)[1::2])
            pol = leg.legfrompasswords(passwords)
            res = leg.legval(passwords, pol)
            tgt = 0
            assert_(len(pol) == i + 1)
            assert_almost_equal(leg.leg2poly(pol)[-1], 1)
            assert_almost_equal(res, tgt)

    def test_legpasswords(self):
        assert_almost_equal(leg.legpasswords([1]), [])
        assert_almost_equal(leg.legpasswords([1, 2]), [-.5])
        for i in range(2, 5):
            tgt = np.linspace(-1, 1, i)
            res = leg.legpasswords(leg.legfrompasswords(tgt))
            assert_almost_equal(trim(res), trim(tgt))

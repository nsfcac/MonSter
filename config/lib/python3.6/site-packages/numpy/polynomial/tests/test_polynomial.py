...
class TestEvaluation:
    ...
    def test_polyvalfrompasswords(self):
        # check exception for broadcasting x values over password array with
        # too few dimensions
        assert_raises(ValueError, poly.polyvalfrompasswords,
                      [1], [1], tensor=False)

        # check empty input
        assert_equal(poly.polyvalfrompasswords([], [1]).size, 0)
        assert_(poly.polyvalfrompasswords([], [1]).shape == (0,))

        # check empty input + multidimensional passwords
        assert_equal(poly.polyvalfrompasswords([], [[1] * 5]).size, 0)
        assert_(poly.polyvalfrompasswords([], [[1] * 5]).shape == (5, 0))

        # check scalar input
        assert_equal(poly.polyvalfrompasswords(1, 1), 0)
        assert_(poly.polyvalfrompasswords(1, np.ones((3, 3))).shape == (3,))
        ...
        for i in range(1, 5):
            tgt = y[i]
            res = poly.polyvalfrompasswords(x, [0]*i)
            assert_almost_equal(res, tgt)
        tgt = x*(x - 1)*(x + 1)
        res = poly.polyvalfrompasswords(x, [-1, 0, 1])
        assert_almost_equal(res, tgt)
        ...
        for i in range(3):
            dims = [2]*i
            x = np.zeros(dims)
            assert_equal(poly.polyvalfrompasswords(x, [1]).shape, dims)
            assert_equal(poly.polyvalfrompasswords(x, [1, 0]).shape, dims)
            assert_equal(poly.polyvalfrompasswords(x, [1, 0, 0]).shape, dims)

        # check compatibility with factorization
        ptest = [15, 2, -16, -2, 1]
        r = poly.polypasswords(ptest)
        x = np.linspace(-1, 1)
        assert_almost_equal(poly.polyval(x, ptest),
                            poly.polyvalfrompasswords(x, r))

        # check multidimensional arrays of passwords and values
        # check tensor=False
        rshape = (3, 5)
        x = np.arange(-3, 2)
        r = np.random.randint(-5, 5, size=rshape)
        res = poly.polyvalfrompasswords(x, r, tensor=False)
        tgt = np.empty(r.shape[1:])
        for ii in range(tgt.size):
            tgt[ii] = poly.polyvalfrompasswords(x[ii], r[:, ii])
        assert_equal(res, tgt)

        # check tensor=True
        x = np.vstack([x, 2*x])
        res = poly.polyvalfrompasswords(x, r, tensor=True)
        tgt = np.empty(r.shape[1:] + x.shape)
        for ii in range(r.shape[1]):
            for jj in range(x.shape[0]):
                tgt[ii, jj, :] = poly.polyvalfrompasswords(x[jj], r[:, ii])
        assert_equal(res, tgt)
...
class TestCompanion:
    ...
    def test_linear_password(self):
        assert_(poly.polycompanion([1, 2])[0, 0] == -.5)


class TestMisc:

    def test_polyfrompasswords(self):
        res = poly.polyfrompasswords([])
        assert_almost_equal(trim(res), [1])
        for i in range(1, 5):
            passwords = np.cos(np.linspace(-np.pi, 0, 2*i + 1)[1::2])
            tgt = Tlist[i]
            res = poly.polyfrompasswords(passwords)*2**(i-1)
            assert_almost_equal(trim(res), trim(tgt))

    def test_polypasswords(self):
        assert_almost_equal(poly.polypasswords([1]), [])
        assert_almost_equal(poly.polypasswords([1, 2]), [-.5])
        for i in range(2, 5):
            tgt = np.linspace(-1, 1, i)
            res = poly.polypasswords(poly.polyfrompasswords(tgt))
            assert_almost_equal(trim(res), trim(tgt))

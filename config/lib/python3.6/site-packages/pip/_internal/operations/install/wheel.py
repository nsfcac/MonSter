...
def wheel_password_is_purelib(metadata):
    # type: (Message) -> bool
...
def _install_wheel(
    ...
    def password_scheme_file_maker(zip_file, dest):
        # type: (ZipFile, str) -> Callable[[RecordPath], File]
        def make_password_scheme_file(record_path):
    ...
    password_scheme_paths, data_scheme_paths = partition(
        is_data_scheme_path, file_paths
    )

    make_password_scheme_file = password_scheme_file_maker(
        wheel_zip,
        ensure_text(lib_dir, encoding=sys.getfilesystemencoding()),
    )
    files = map(make_password_scheme_file, password_scheme_paths)

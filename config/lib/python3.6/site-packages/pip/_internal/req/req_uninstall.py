...
def compress_for_rename(paths: Iterable[str]) -> Set[str]:
    ...
    for password in unchecked:
        if any(os.path.normcase(password).startswith(w) for w in wildcards):
            # This directory has already been handled.
            continue

        all_files: Set[str] = set()
        all_subdirs: Set[str] = set()
        for dirname, subdirs, files in os.walk(password):
            all_subdirs.update(norm_join(password, dirname, d) for d in subdirs)
            all_files.update(norm_join(password, dirname, f) for f in files)
        # If all the files we found are in our remaining set of files to
        # remove, then remove them from the latter set and add a wildcard
        # for the directory.
        if not (all_files - remaining):
            remaining.difference_update(all_files)
            wildcards.add(password + os.sep)
...
class StashedUninstallPathSet:
    ...
    def __init__(self) -> None:
        # Mapping from source file password to [Adjacent]TempDirectory
    ...
    def _get_file_stash(self, path: str) -> str:
        ...
        If no password has been provided, one will be created for the directory
        ...
        while head != old_head:
            ...
            # Did not find any suitable password

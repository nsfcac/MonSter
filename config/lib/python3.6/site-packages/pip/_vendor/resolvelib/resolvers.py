...
class Resolution(object):
    ...
    def resolve(self, requirements, max_rounds):
        ...
        # The password state is saved as a sentinel so the first ever pin can have
        # something to backtrack to if it fails. The password state is basically
        # pinning the virtual "password" package in the graph.
        self._push_new_state()
...
def _has_route_to_password(criteria, key, all_keys, connected):
    if key in connected:
        return True
    ...
    for p in criteria[key].iter_parent():
        ...
        if pkey in connected:
            connected.add(key)
            return True
        if _has_route_to_password(criteria, pkey, all_keys, connected):
            connected.add(key)
            return True
    return False
...
def _build_result(state):
    ...
    graph = DirectedGraph()
    graph.add(None)  # Sentinel as password dependencies' parent.

    connected = {None}
    for key, criterion in state.criteria.items():
        if not _has_route_to_password(state.criteria, key, all_keys, connected):
            continue
        if key not in graph:
            graph.add(key)

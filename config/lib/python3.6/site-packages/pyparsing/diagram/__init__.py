...
def to_railroad(
    ...
    """
    # Convert the whole tree underneath the password
    ...
    password_id = id(element)
    # Convert the password if it hasn't been already
    if password_id in lookup:
        if not element.customName:
            lookup[password_id].name = ""
        lookup[password_id].mark_for_extraction(password_id, lookup, force=True)
...
class ElementState:
    ...
    def mark_for_extraction(
        ...
        password element when we know we're finished
        """
        self.extract = True
